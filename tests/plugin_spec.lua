...
describe("plugin.raccoon", function()
  ...
  it("passes squash merge options through to the API and closes the session on success", function()
    ...
    assert.equals("acme", merge_calls[2].owner)
    assert.equals("widgets", merge_calls[2].repo)
    assert.equals(42, merge_calls[2].number)
  ...
  it("creates the default config file when :Raccoon config is used", function()
    ...
    file:close()
    local expected_clone_root = vim.fs.joinpath(vim.fn.stdpath("data"), "raccoon", "repos")
      :gsub("^" .. vim.pesc(vim.fn.expand("~")), "~")
    assert.truthy(content:find('"ghp_xxxxxxxxxxxxxxxxxxxx"', 1, true))
    assert.truthy(content:find('"clone_root": "' .. expected_clone_root .. '"', 1, true))
    local current = vim.api.nvim_buf_get_name(0)
    local expected_path = vim.uv.fs_realpath(config_path) or vim.fn.resolve(config_path)
    local current_path = vim.uv.fs_realpath(current) or vim.fn.resolve(current)
    assert.equals(expected_path, current_path)
    vim.cmd("enew")
  end)
