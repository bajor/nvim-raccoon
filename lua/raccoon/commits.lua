...
local function setup_keymaps()
  ...
  if config.is_enabled(shortcuts.commit_mode.maximize_prefix) then
    ...
    table.insert(commit_mode_keymaps, {
      ...
      rhs = function()
        local items = {}
        for path, _ in pairs(commit_state.commit_files) do
        ...
        ui.open_maximize_list({
          ...
          on_select = function(path)
            for i, hd in ipairs(commit_state.all_hunks) do
              if hd.filename == path then
                local c = commit_state.grid_rows * commit_state.grid_cols
                commit_state.current_page = math.ceil(i / c)
                render_grid_page()
      ...
      rhs = function()
        ...
        ui.open_maximize_list({
          ...
          on_select = function(idx)
            commit_state.selected_index = idx
            update_sidebar_selection()
            select_commit(idx)
          end,
        })
