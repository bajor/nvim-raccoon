...
Flat, commit, and local diff views use the same exact-span renderer. Character spans match Pierre's `Char` mode through a pure-Lua adaptation of the Myers algorithm in `diff@9.0.0`. Within each contiguous change block, deletion and addition lines pair by ordinal position; context lines start new blocks, and surplus lines remain fully highlighted. Pairs up to 1,000 UTF-16 units per side receive exact spans. A pair with either side above that limit keeps only subdued row backgrounds, without preventing later pairs from receiving exact spans.

Raccoon's separate 500-millisecond deadline applies to the complete file or visible grid page. If that deadline is reached, the entire view uses stronger whole-line highlights and discards all partial exact spans. Flat-review deleted virtual lines retain their complete text.

GitHub patch bodies are checked against their hunk coordinates and reported addition/deletion totals. Initial open and sync pin the checkout to the exact PR head SHA and recheck the head and base after loading mutable API data; if either changed, the operation stops instead of combining different snapshots. To rebuild an incomplete or omitted text patch, Raccoon lazily completes shallow history, fetches the base branch, resolves immutable merge-base/head revisions, and diffs the affected path (both paths for a rename). If recovery fails, it warns whenever the file opens and renders no partial diff. GitHub's pull-request-files endpoint returns at most 3,000 files; when the PR's changed-file count exceeds the returned list, Raccoon warns that the omitted files are unavailable because their filenames are absent from the response. Fully removed files open as an empty post-image with their deleted lines, while binary and metadata-only changes are explicitly identified as having no textual diff. In commit and local previews, a literal unhighlighted `...` appears only between disjoint complete hunks and means that known unchanged lines were omitted from the preview.
...
- Raccoon creates true line-level review threads only where GitHub's public PAT-authenticated review comment APIs can resolve them: changed lines and unchanged context lines already present in the PR diff. If you start a new thread on another line of a changed file, raccoon falls back to a file-level REST comment with `subject_type=file`. GitHub renders those at file scope, so raccoon mirrors them at the top of the file instead of pretending they are line comments. GitHub's newer web UI can comment on more unchanged lines than the public APIs raccoon uses today.
