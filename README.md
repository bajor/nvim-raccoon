...
The default comparison follows `@pierre/diffs` 1.2.12's `word-alt` behavior: words, punctuation, and whitespace are matched with the same `diff@9.0.0` algorithm, including its single-unit neutral-span joining. Structured identifiers such as `old_timeout` and `new_timeout` are refined with that release's character matcher when enough unchanged content makes the result more precise; short version-like identifiers such as `v1` and `v2` are also refined. Ordinary prose remains word-oriented.

Replacement rows first use Neovim's built-in `vim.diff` line matching so an inserted or removed row does not shift every later pair. If that alignment leaves fewer pairs than the block can support, Raccoon falls back to `@pierre/diffs` 1.2.12's positional row pairing for that block. Excess add-only or delete-only rows retain only their whole-line background. A line over the comparison limit disables inline ranges only for its pair, not for neighboring pairs. Extmark ranges use UTF-8 byte columns, so tabs and multibyte text do not shift the highlighted bytes.

The result is intentionally Pierre-compatible rather than byte-for-byte identical to the browser renderer. Pierre's default stays word-level inside identifiers and always pairs block rows positionally; Raccoon adds identifier refinement and prefers `vim.diff` alignment before using Pierre's behavior as a coverage fallback. Neovim syntax highlighting and byte-based extmarks also replace Pierre's Shiki and UTF-16/DOM rendering layers.
