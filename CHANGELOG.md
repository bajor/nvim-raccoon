...
- The local commit viewer's file picker (`<leader>mf`) now opens the chosen file centered on its first hunk, because it maximizes that hunk's grid cell.
...
### Added
- GitHub Actions workflow caller for the shared visual explanation SVG cleanup workflow.
...
- New review threads now allow unchanged lines that are still inside GitHub's PR diff context, and raccoon blocks non-diff lines before they fail with a late `422`.
...
### Fixed
- Grid column widths are now equalized after sidebar resize — explicitly computes column width from available space instead of relying on `wincmd =`
